...
/*   Updated: 2022/09/02 16:58:26 by iomayr           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_command	*ft_parse(t_tokens_list *tokens_list, t_env *h_env)
{
	...
	while (var.current_token != NULL)
	{
		...
		if (var.current_token->type == PIPE
			|| var.current_token->type == ENEWLINE)
			set_cmd(&var);
		if (var.current_token->type == REDIR_GREATER
			|| var.current_token->type == REDIR_LESSER
			|| var.current_token->type == DOUBLE_GREATER
			|| var.current_token->type == DOUBLE_LESSER)
		{
